...
def main() -> None:
    ...
    print(f"Downloading data for {len(tickers)} tickers â†’ {OUTPUT_DIR}")
    ...
    for i, ticker in enumerate(tickers, 1):
        try:
            ...
            print(f"  [{i}/{len(tickers)}] {ticker} â†’ {path}")
        except Exception as exc:
            print(f"  [{i}/{len(tickers)}] {ticker} FAILED: {exc}")
            failed.append(ticker)

    if failed:
        print(f"\nâš   Failed tickers ({len(failed)}): {', '.join(failed)}")

    # Validate
    print("\nValidating downloaded dataâ€¦")
