...
1. Push to `main` branch â€” CD workflow triggers automatically.
...
docker build -t your-registry/stocktrader-api:staging .
docker push your-registry/stocktrader-api:staging
...
kubectl set image deployment/stocktrader-api \
  api=your-registry/stocktrader-api:staging \
  -n stocktrader-staging
```
...
1. Update secrets in your secret manager (Vault / AWS Secrets Manager / GCP Secret Manager).
2. Restart the application to pick up new values:
   ```bash
   # Docker Compose
   docker-compose -f docker-compose.prod.yml restart api

   # Kubernetes
   kubectl rollout restart deployment/stocktrader-api -n stocktrader-prod
   ```
...
1. Set `PAPER_MODE=true` in environment (if not already):
   ```bash
   # Docker
   docker exec stocktrader-api sh -c "export PAPER_MODE=true"
   docker-compose restart api

   # Kubernetes
   kubectl set env deployment/stocktrader-api PAPER_MODE=true -n stocktrader-prod
   ```
