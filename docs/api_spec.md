...
**Auth:** None  
**Tags:** `prediction`  
**Schema:** `PredictRequest` â†’ `PredictResponse`

#### Request Body

| Field | Type | Required | Constraints | Description |
|-------|------|----------|-------------|-------------|
| `ticker` | string | yes | 1â€“10 chars | Stock ticker symbol |
| `horizon_days` | integer | no (default 5) | 1â€“365 | Calendar days to predict ahead |
...
| `confidence` | float (0â€“1) | Model confidence score |
...
**Auth:** None  
**Tags:** `prediction`  
**Schema:** `BatchPredictRequest` â†’ `BatchPredictResponse`

#### Request Body

| Field | Type | Required | Constraints | Description |
|-------|------|----------|-------------|-------------|
| `tickers` | string[] | yes | 1â€“50 items | List of ticker symbols |
| `horizon_days` | integer | no (default 5) | 1â€“365 | Calendar days to predict ahead |
...
| `accuracy` | float \| null | Latest evaluation accuracy (0â€“1) |
...
**Tags:** `model`  
**Schema:** `ModelReloadRequest` â†’ `ModelReloadResponse`
...
**Auth:** None  
**Tags:** `trading`  
**Schema:** `TradeIntentRequest` â†’ `TradeIntentResponse`

#### Request Body

| Field | Type | Required | Constraints | Description |
|-------|------|----------|-------------|-------------|
| `ticker` | string | yes | 1â€“10 chars | Ticker symbol |
| `side` | string | yes | `buy` \| `sell` | Order side |
| `quantity` | integer | yes | 1â€“100,000 | Number of shares |
...
| Status | Code | When |
|--------|------|------|
| 400 | `VALIDATION_ERROR` | Invalid ticker, quantity â‰¤ 0, missing limit_price for limit order |
...
**Tags:** `trading`  
**Schema:** `ExecuteRequest` â†’ `ExecuteResponse`
...
**Auth:** None  
**Tags:** `backtest`  
**Schema:** `BacktestRunRequest` â†’ `BacktestRunResponse`

#### Request Body

| Field | Type | Required | Constraints | Description |
|-------|------|----------|-------------|-------------|
| `tickers` | string[] | yes | 1â€“50 items | Tickers to include in simulation |
...
| `strategy` | string | no (default `momentum`) | â€” | Strategy identifier |
...
**Auth:** None  
**Tags:** `backtest`  
**Schema:** â†’ `BacktestResultsResponse`
...
| 202 | â€” | Job still running (body contains `{"job_id": "...", "status": "running"}`) |
...
|-------|------|-------------|-------------|
| `ticker` | string | â€” | Stock ticker symbol |
| `action` | enum | `buy` \| `sell` \| `hold` | Recommended action |
| `confidence` | float | 0.0â€“1.0 | Model confidence |
| `expected_return` | float | â€” | Expected % return over horizon |
