...
@pytest.fixture()
def setup_fixture_as_raw(tmp_path):
    """Copy sample OHLCV CSV into a temp raw dir with a matching ticker name."""
    src = FIXTURES_DIR / "sample_ohlcv.csv"
    dest = tmp_path / "TESTICKER.csv"
    dest.write_text(src.read_text())
    return tmp_path


def test_build_features_columns(setup_fixture_as_raw):
    df = build_features(["TESTICKER"], data_dir=setup_fixture_as_raw)
