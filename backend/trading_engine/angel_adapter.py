...
"""Angel One SmartAPI adapter â€“ paper + live implementations.
...
Credentials (set in ``.env``):
    ANGEL_API_KEY      â€“ SmartAPI key from https://smartapi.angelone.in/
    ANGEL_CLIENT_ID    â€“ Your Angel One client ID
    ANGEL_MPIN         â€“ 4-digit MPIN for login
    ANGEL_TOTP_SECRET  â€“ TOTP secret for 2FA
"""
...
_DEFAULT_PAPER_BALANCE = float(os.getenv("PAPER_BALANCE", "100000"))
...
class AngelPaperAdapter:
    ...
    ``.env`` to change the starting amount (default â‚¹1,00,000).
    """
    ...
    def _execute(self, intent: dict) -> dict:
        ...
        # Update balance: buy â†’ lock cash, sell â†’ release cash
...
# ---------------------------------------------------------------------------
# Factory â€“ auto-selects adapter based on PAPER_MODE
# ---------------------------------------------------------------------------
