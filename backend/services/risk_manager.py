...
class RiskConfig:
    ...
    max_daily_loss: float = 5_000.0     # Hard daily loss limit (â‚¹)
...
class RiskManager:
    ...
    def update_capital(self, available_cash: float) -> None:
        ...
        logger.debug("RiskManager capital updated to â‚¹%.2f", available_cash)
    ...
    def can_open_position(self, ticker: str, price: float, quantity: int) -> tuple[bool, str]:
        ...
        if self.daily_pnl <= -daily_limit:
            return False, f"Daily loss limit reached (â‚¹{daily_limit:.0f})"
        ...
        if position_value > max_position:
            return False, f"Position â‚¹{position_value:.0f} exceeds max â‚¹{max_position:.0f}"
        ...
        if total_exposure > max_exposure:
            return False, f"Portfolio exposure â‚¹{total_exposure:.0f} would exceed max â‚¹{max_exposure:.0f}"
