...
class CanaryEvaluator:
    ...
    Workflow
    --------
    1. Register a candidate model version â†’ starts in SHADOW stage.
    ...
    def evaluate_shadow(self) -> CanaryStage:
        """Decide whether to promote from SHADOW â†’ CANARY."""
        ...
        if acc_diff > self.config.max_accuracy_drop:
            ...
            logger.info("Shadow PASSED â†’ promoting to CANARY")
    ...
    def evaluate_canary(self) -> CanaryStage:
        ...
        if acc_diff > self.config.max_accuracy_drop:
            ...
            logger.info("Canary PASSED â†’ PROMOTED candidate %s", self.candidate.version)
