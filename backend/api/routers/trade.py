...
# In-memory intent store (thread-safe)
_intents: dict[str, dict] = {}
_intents_lock = threading.Lock()


def _get_adapter():
    """Lazy adapter accessor â€“ avoids crash on import if credentials are bad."""
...
async def execute(req: ExecuteRequest, token: str = Depends(_require_auth)):
    ...
    with _intents_lock:
        ...
        intent = _intents[intent_id].copy()
    ...
    with _intents_lock:
        if intent_id in _intents:
            _intents[intent_id]["status"] = "filled"
